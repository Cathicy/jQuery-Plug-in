(function(window,undefined){
    var yjQuery = function(selector){
        return new yjQuery.prototype.init(selector);
    }
    yjQuery.prototype = {
        constructor: yjQuery,
        init: function (selector) {
            // 0.去除字符串两端的空格
            selector = yjQuery.trim(selector);
            // 1.传入 '' null undefined NaN  0  false, 返回空的jQuery对象
            if(!selector){
                return this;
            }
            // 2.方法处理
            else if(yjQuery.isFunction(selector)){
                yjQuery.ready(selector);
            }
            // 3.字符串
            else if(yjQuery.isString(selector)){
                // 2.1判断是否是代码片段 <a>
                if(yjQuery.isHTML(selector)){
                    // 1.根据代码片段创建所有的元素
            ...
            // 4.数组
            else if(yjQuery.isArray(selector)){
                // 转换为真数组
        ...
        each:function(fn){
            return yjQuery.each(this,fn);
        }
    }


    yjQuery.extend = yjQuery.prototype.extend = function (obj) {
        for(var key in obj){
            this[key] = obj[key];
        }
    }
    ...
    yjQuery.extend({
        ...
        isArray: function(sele){
            if(yjQuery.isObject(sele) && !yjQuery.isWindow(sele) && "length" in sele){
                return true;
            }
            return false;
        },
        ...
        clone: function (deep) {
            ...
            if(deep){
                ...
                this.each(function (key, ele) {
                    ...
                    // 遍历元素中的eventsCache对象
                    yjQuery.each(ele.eventsCache, function (name, array) {
                        // 遍历事件对应的数组
                        yjQuery.each(array, function (index, method) {
                            // 给复制的元素添加事件
                            $(temp).on(name, method);
                        });
                    });
    ...
    yjQuery.prototype.extend({
        next: function(sele){
            ...
            if(arguments.length === 0){
                //返回所有找到的
                this.each(function(key,value){
                    var temp = yjQuery.get_nextsibling(value);
                    if(temp != null){
                        res.push(temp);
                    }
                });
        ...
        prev: function(sele){
            ...
            if(arguments.length === 0){
                this.each(function(key,value){
                    var temp = yjQuery.get_previoussibling(value);
    ...
    yjQuery.prototype.extend({
        ...
        css: function(attr,value){
            ...
            if(yjQuery.isString(attr)){
                // 判断是一个字符串还是两个字符串
                if(arguments.length === 1){
                    return yjQuery.getStyle(this[0], attr);
                }else{
                    this.each(function(key,ele){
    ...
    yjQuery.prototype.extend({
        on: function(name,callBack){
            ...
            this.each(function (key,ele) {
                ...
                if(!ele.eventsCache[name]){
                    ...
                    //5.添加对应类型的事件
                    yjQuery.addEvent(ele,name,function(){
                        yjQuery.each(ele.eventsCache[name],function (k,method) {
                            method.call(ele);
                        });
                    });
                }else{
        ...
        off: function(name,callBack){
            ...
            //3.判断是否传入了两个参数
                ...
                this.each(function (key,ele) {
                    yjQuery.each(ele.eventsCache[name],function (index,method) {
    ...
    yjQuery.prototype.init.prototype = yjQuery.prototype;
    window.yjQuery = window.$ = yjQuery;
})(window);